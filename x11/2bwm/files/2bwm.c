...
static void setunfocus(struct client *);
...
drawborders(void)
{
	...
	if (focuswin != NULL) setborders(focuswin, true);
}
...
void setunfocus(struct client *win)
{
//    xcb_set_input_focus(conn, XCB_NONE, XCB_INPUT_FOCUS_NONE,XCB_CURRENT_TIME);
	if (NULL == win||win->id == screen->root)
		return;

	setborders(win, false);
}
...
setfocus(struct client *client)// Set focus on window client.
{
	...
	if (NULL != focuswin)
		setunfocus(focuswin); /* Unset last focus. */
...
cursor_move(const Arg *arg)
{
	...
	if      (cases == TWOBWM_CURSOR_UP)    xcb_warp_pointer(conn, XCB_NONE, XCB_NONE, 0, 0, 0, 0, 0, -speed);
	else if (cases == TWOBWM_CURSOR_DOWN)  xcb_warp_pointer(conn, XCB_NONE, XCB_NONE, 0, 0, 0, 0, 0, speed);
	else if (cases == TWOBWM_CURSOR_RIGHT) xcb_warp_pointer(conn, XCB_NONE, XCB_NONE, 0, 0, 0, 0, speed, 0);
	else if (cases == TWOBWM_CURSOR_LEFT)  xcb_warp_pointer(conn, XCB_NONE, XCB_NONE, 0, 0, 0, 0, -speed, 0);
...
enternotify(xcb_generic_event_t *ev)
{
	...
	if (e->mode == XCB_NOTIFY_MODE_NORMAL
		...
		/* Set focus to the window we just entered if we
...
void
leavenotify(xcb_generic_event_t *ev)
{
	xcb_enter_notify_event_t *e = (xcb_enter_notify_event_t *)ev;
	struct client *client;
...
setup(int scrno)
{
	...
	events[XCB_LEAVE_NOTIFY]        = leavenotify;
